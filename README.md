®×õj{"j{"r˜5Ûœ¡Ê®×öó®\¡Êµìm±Æ¥Šx4r‡(j™Zµú+›GèÛ[•ı³¬±÷!j»·AŞ–÷­‰Æœ¢Z+µ¹kyİ¹æ
Şz}¹å¹n{ny{Z×r‰h®Öå™ªà—^8Òf«‚½xãKâ{5×´¾'°‡Î4ÒøÂH§wJZ­Ûqïm-Ç^8ÒÜv×­-Ço<ÒÜwëM-ÇöÒÜyÓ-ÇúÒÜzãÍ-Ç½´ÒÜ{÷m-ÇÎ¸Ò–«v*çjÛ«jZZ®Ø †×§}§-¢½Ñû6áÇô# Evaluation Agent
...
- `GET /api/configs` â€” preset configurations exposed by the dashboard
- `GET /api/runs` â€” list of historical runs stored in `runs/evaluations.db`
- `POST /api/runs` â€” execute a configuration immediately
- `GET /api/runs/{id}` â€” retrieve metrics and predictions for a single run
...
4. Deploy the services:
   - **`eval-agent-api`** â€” Python web service that runs `uvicorn eval_agent.api.app:app`.
     The build step installs this package via `pip install .`, and the service exposes the HTTP
     API on the free tier.
   - **`eval-agent-dashboard`** â€” Static site that runs `npm install && npm run build` inside
...
write to `runs/evaluations.db`, which lives on the instance's ephemeral diskâ€”data will be cleared
